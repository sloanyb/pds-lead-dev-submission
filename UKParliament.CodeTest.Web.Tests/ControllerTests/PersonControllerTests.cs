...
public class PersonControllerTests
{
    ...
    public async Task WhenValidPersonIsAdded_ReturnsCreatedAtActionResult()
    {
        ...
        A.CallTo(() => personService.AddPersonAsync(A<Person>.Ignored))
            .ReturnsLazily((Person p) =>
            {
                ...
                {
                    Id = p.DepartmentId,
                    Name = p.DepartmentId == 1 ? "Sales" :
                        p.DepartmentId == 2 ? "Marketing" :
                        p.DepartmentId == 3 ? "Finance" : "HR"
                };
                return Task.FromResult(p);
            });

        var newPersonViewModel = new PersonAddViewModel
        {
            FirstName = "Alice",
            LastName = "Smith",
        ...
        A.CallTo(() => personService.AddPersonAsync(A<Person>.That.Matches(
                p => p.FirstName == "Alice" && p.LastName == "Smith" && p.DateOfBirth == new DateOnly(1990, 5, 15) &&
                     p.DepartmentId == 1)))
    ...
    public async Task WhenGetAllCalled_ReturnsAllPeople()
    {
        ...
        Assert.Contains(returnedPeopleViewModels,
            vm => vm.Id == 1 && vm.FirstName == "Alice" && vm.LastName == "Smith" &&
                  vm.DateOfBirth == new DateOnly(1990, 1, 1) && vm.Department.Id == 1 &&
                  vm.Department.DepartmentName == "Sales");
        Assert.Contains(returnedPeopleViewModels,
            vm => vm.Id == 2 && vm.FirstName == "Bob" && vm.LastName == "Jones" &&
                  vm.DateOfBirth == new DateOnly(1985, 2, 2) && vm.Department.Id == 2 &&
                  vm.Department.DepartmentName == "Marketing");
    }
    ...
    public async Task WhenValidPersonIsUpdated_ReturnsUpdatedPerson()
    {
        ...
        A.CallTo(() => personService.UpdatePersonAsync(A<Person>.That.Matches(
                p => p.Id == 1 && p.FirstName == "Joseph" && p.LastName == "Bloggs" &&
                     p.DateOfBirth == new DateOnly(1980, 1, 1) && p.DepartmentId == 1)))
    ...
    {
        var personServiceFake = A.Fake<IPersonService>();
        
        var controller = new PersonController(personServiceFake);
        var invalidViewModel = new PersonAddViewModel();
        ...
        controller.ModelState.AddModelError("DateOfBirth", "The DateOfBirth field is required.");
        controller.ModelState.AddModelError("DepartmentId", "The DepartmentId field is required.");
        
        var result = await controller.Add(invalidViewModel);
